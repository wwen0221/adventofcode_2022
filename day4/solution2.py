...
def get_pairs(pairs):
    ...
    if (second_min >= first_min  and second_min <= first_max):
        if (first_max >= second_min  and first_max <= second_max) or \
            (second_max >= first_min  and second_max <= first_max):
            return 1

    elif (first_min >= second_min  and first_min <= second_max):
        if (second_max >= first_min  and second_max <= first_max) or \
...
if __name__ == '__main__':
    ...
    df.columns = ['pairs']
    df['is_overlap'] = df['pairs'].apply(lambda x: get_pairs(x))

    print(df['is_overlap'].sum())
